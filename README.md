...
- **Multiverse-Core takes over world loading.** On first start it writes
  `plugins/Multiverse-Core/worlds.yml` and imports the existing `world` as its default. Worlds you
  create through it appear as new top-level directories beside `world/`, so include them when you
  back up.
